...
_R.NPCFaction.Create("NPC_FACTION_PLAYER","npc_horsey")
ENT.NPCFaction = NPC_FACTION_PLAYER
ENT.iClass = CLASS_PLAYER_ALLY
util.AddNPCClassAlly(CLASS_PLAYER_ALLY,"npc_horsey")
...
ENT.CollisionBounds = Vector(35,10,44)
...
function ENT:Mount(pl)
	...
	pl:SetPos(pos +self:GetUp() *50 +self:GetForward() *6)
	pl:SetParent(self)
...
function ENT:MountThink()
	...
	if(pl:KeyDown(IN_USE) && CurTime() -self.m_tMounted > 0.5) then
		self:Dismount()
		return
	end
...
function ENT:OnTakeDamage(dmginfo)
	if dmginfo:IsFallDamage() == true then
