PLUGIN.name = "NPCs+"
PLUGIN.author = "AleXXX_007, Black Tea (NS 1.0), Neon (NS 1.1)"
PLUGIN.desc = "Adding talking NPCs with trade function."
...
if (SERVER) then
	...
	netstream.Hook("vendor", function(ent)
		ent.receivers[#ent.receivers + 1] = LocalPlayer():GetName()
		
		local items = {}

		-- Only send what is needed.
		for k, v in pairs(ent.items) do
			if (table.Count(v) > 0 and (LocalPlayer():IsAdmin() or v[VENDOR_MODE])) then
				items[k] = v
			end
		end

		ent.scale = ent.scale or 0.5
		...
		if (LocalPlayer():GetName():IsAdmin()) then
			data[4] = ent.messages
			data[5] = ent.factions
			data[6] = ent.classes
		end

		LocalPlayer():GetName().nutVendor = ent
		netstream.Start(LocalPlayer():GetName(), "vendorOpen", ent:EntIndex(), unpack(data))
	end)
else
	VENDOR_TEXT = {}
