local PANEL = {}
	function PANEL:Init()
		...
		upgrade:Dock(TOP)
		upgrade:SetText(L"stashUpgrade")
		upgrade.DoClick = function()
		Derma_Query(Format("Увеличить вместимость хранилища на 2. Это будет стоить вам %s.", nut.currency.get(LocalPlayer():getChar():getUpgradeCost())), L"stashTitle1", L"yes", function()
		...
		self.inv.action.DoClick = function()
			...
			if (IsValid(selectedItem)) then
				if LocalPlayer():getChar():getNumStash() < LocalPlayer():getChar():getMaxStash() then
					netstream.Start("stashIn", selectedItem.indexID, true)
				else
					nut.util.notify("Ваше хранилище переполнено!")
				end
			end
		end
	end
	...
	function PANEL:setStash()		
		self.stash.title.Think = function()
			self.stash.title:SetText(L("stash", 0, 50).." ("..LocalPlayer():getChar():getNumStash().."/"..LocalPlayer():getChar():getMaxStash()..")")
		end
