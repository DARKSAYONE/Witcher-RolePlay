...
if (SERVER) then
	...
	function ENT:Use(activator)
		...
		if ((wep == "nut_dagger_iron") or (wep == "nut_dagger_ironknife") or (wep == "nut_dagger_steel") or (wep == "nut_dagger_steelknife"))  then
			if (wep == "nut_dagger_iron") then
				attribTime = attribTime + 10
				chance = chance - 10
			elseif (wep == "nut_dagger_ironknife") then
				attribTime = attribTime + 5
				chance = chance - 5
			elseif (wep == "nut_dagger_steel") then
				attribTime = attribTime/1.5
				chance = chance + 5
			elseif (wep == "nut_dagger_ironknife") then
				attribTime = attribTime/2
				chance = chance + 10
			end
			if (activator:KeyDown(IN_DUCK)) then
				...
				activator:setAction("Разделываем...", attribTime, function()
					if (activator:KeyDown(IN_DUCK)) and (activator:GetPos():Distance(self:GetPos()) <= 2000) then
						if (chance > 60) and (chance < 80) then
							...
							elseif (animal == 4) then
								activator:getChar():getInv():add("wolf_meat")
							end
						...
						elseif (chance > 80) and (chance < 100) then
							...
							elseif (animal == 3) then
								activator:getChar():getInv():add("wolf_meat")
								activator:getChar():getInv():add("wolf_hide")
							end
						...
						elseif (chance > 100) then
							...
							elseif (animal == 3) then
								activator:getChar():getInv():add("wolf_meat")
								activator:getChar():getInv():add("wolf_hide")
								activator:getChar():getInv():add("wolf_klik")
							end
