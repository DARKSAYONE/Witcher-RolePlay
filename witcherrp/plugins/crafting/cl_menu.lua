...
hook.Add("CreateMenuButtons", "nutCrafting", function(tabs)
	tabs["Ремесло"] = function(panel)
		if (!IsValid(nut.gui.crafting)) then
			netstream.Start("HandCrafting", LocalPlayer())
			local crafting = panel:Add("nut_Crafting")
			crafting:SetPos(ScrW()*0.25, ScrH()*0.25)
			crafting:ShowCloseButton(false)
		end
	end
end)