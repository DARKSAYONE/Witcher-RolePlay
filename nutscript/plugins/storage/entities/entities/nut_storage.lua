...
if (SERVER) then
	...
	function ENT:OpenInv(activator)
		...
		local def = PLUGIN.definitions[self:GetModel():lower()] or {
		name = "Мешок",
		desc = "Мешок",
		width = nut.config.get("invW"),
		height = nut.config.get("invH"),
	}
	...
	function ENT:onDrawEntityInfo(alpha)
		...
		local tx, ty = drawText("Контейнер", x, y, colorAlpha(configGet("color"), alpha), 1, 1, nil, alpha * 0.65)
		if (def) then
			y = y + ty + 1
