...
if (SERVER) then
	...
	function PLUGIN:PlayerDisconnected(client)
		nut.log.add(client:steamName().." ("..client:SteamID()..") disconnected.")
	end

	function PLUGIN:PlayerInitialSpawn(client)
		nut.log.add(client:steamName().." ("..client:SteamID()..") connected.")
	end	
	...
	function PLUGIN:OnPlayerInteractItem(client, action, item)
		if (logInteractions[action:lower()]) then
			...
			nut.log.add(client, L("used \"%s\" on %s.", action, item.uniqueID))
		end
	end
end