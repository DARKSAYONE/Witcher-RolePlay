...
if (CLIENT) then
	...
	local NUT_CVAR_TP_DIST = CreateClientConVar("nut_tp_distance", 100, true)
	...
	function PANEL:Init()
		self:SetTitle("Настройка вида от третьего лица")
		...
		cfg:Dock(TOP)
		cfg:SetText("Высота") // Set the text above the slider
		...
		local cfg = self.list:Add("DNumSlider")
		cfg:Dock(TOP)
		cfg:SetText("Горизонтальное положение") // Set the text above the slider
		...
		local cfg = self.list:Add("DNumSlider")
		cfg:Dock(TOP)
		cfg:SetText("Дальность") // Set the text above the slider
		cfg:SetMin(0)				 // Set the minimum number you can slide to
