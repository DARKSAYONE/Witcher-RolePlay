...
if (CLIENT) then
	...
	function PLUGIN:HUDPaint()
		...
		if (client:IsAdmin() and client:GetMoveType() == MOVETYPE_NOCLIP and !client:InVehicle() and NUT_CVAR_ADMINESP:GetBool()) then
			...
			for k, v in ipairs(player.GetAll()) do
				...
				nut.util.drawText(v:Name().."\n("..v:steamName()..")", x, y - size, ColorAlpha(teamColor, alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, nil, alpha)
			end
		end
	end
	...
	function PLUGIN:SetupQuickMenu(menu)
		if (LocalPlayer():IsAdmin()) then
			...
			local buttonTP = menu:addCheck("Режим наблюдателя", function(panel, state)
				if (state) then
