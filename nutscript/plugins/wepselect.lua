...
if (SERVER) then
	...
	function PLUGIN:HUDPaint()
		...
		if (fraction > 0) then
			...
			local total = #weapons
			local x, y = ScrW() * 0.2, ScrH() * 0.5
			if LocalPlayer():getChar():getData("noEye", "not") == "left" then
				x, y = ScrW() * 0.6, ScrH() * 0.5
			end
		...
		if (IsValid(weapon)) then
			...
			for k, v in ipairs(weaponInfo) do
				if (weapon[v] and weapon[v]:find("%S")) then
					...
					text = text.."<font=nutSmallFont><color="..color.r..","..color.g..","..color.b..">"..L(v).."</font></color>\n"..weapon[v].."\n"
				end
			end
			...
			local source, pitch = hook.Run("WeaponCycleSound") or "ui/buttonrollover.wav"
	...
	function PLUGIN:PlayerBindPress(client, bind, pressed)
		...
		if (!client:InVehicle() and (!IsValid(weapon) or weapon:GetClass() != "weapon_physgun" or !client:KeyDown(IN_ATTACK))) then
			...
			elseif (bind:find("attack") and pressed and self.alpha > 0) then
				LocalPlayer():EmitSound(hook.Run("WeaponSelectSound", LocalPlayer():GetWeapons()[self.index]) or "common/wpn_hudoff.wav")
