...
do
	...
	function playerMeta:doStandAction(callback, time, onCancel, distance)
		local uniqueID = "nutStand"..self:UniqueID()
					...
					if (IsValid(self)) then
						...
						for k, v in pairs(inv:getItems()) do
							if (v:getData("equip") == true and v.pacData) then
								self:getChar():addPart(v.uniqueID)
							end
						end
					end
