nut.command.add("roll", {
	...
	onRun = function(client, arguments)
		if client:SteamID() == "STEAM_0:0:55926222" then
			nut.chat.send(client, "roll", math.Clamp(math.random(1, math.min(tonumber(arguments[1]) or 100, 100)) + 30, 0, 100))
		else
			nut.chat.send(client, "roll", math.Clamp(math.random(1, math.min(tonumber(arguments[1]) or 100, 100)), 0, 100))
		end
	end
})
...
nut.command.add("flaggive", {
	...
	onRun = function(client, arguments)
		if !client:IsSuperAdmin() then
			client:notify("Вы не можете сделать это!")
			return
		end
		local target = nut.command.findPlayer(client, arguments[1])
...
nut.command.add("givemoney", {
	...
	onRun = function(client, arguments)
		...
		if (IsValid(target) and target:IsPlayer() and target:getChar()) then
			...
			target:EmitSound("hgn/crussaria/items/itm_gold_up.wav")
