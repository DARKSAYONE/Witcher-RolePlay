...
local function DisallowSpawnMenu()
	if LocalPlayer():getChar():hasFlags("p") 
	or LocalPlayer():getChar():hasFlags("t") 
	or LocalPlayer():getChar():hasFlags("n")
	or LocalPlayer():getChar():hasFlags("c")
	or LocalPlayer():getChar():hasFlags("C")
	or LocalPlayer():getChar():hasFlags("e")
	or LocalPlayer():getChar():hasFlags("r")
	or LocalPlayer():IsAdmin() then
		return true
	else
		return false
	end
...
function GM:LoadFonts(font)
	if NUT_CVAR_FONT:GetString() == "Ведьмак" then
	...
	surface.CreateFont("nutBattleFont", {
		font = font,
		size = 25,
		weight = 700,
		extended = true,
	...
	surface.CreateFont("nutKeysFont", {
		font = font,
		size = 20,
		weight = 700,
		extended = true,
	...
	surface.CreateFont("nutChatFont1", {
		font = font,
	...
	surface.CreateFont("nutBusiness1", {
		font = font,
		size = ScreenScale(8),
		weight = 600,
		extended = true,
	...
	surface.CreateFont("nutBusiness2", {
		font = font,
		size = ScreenScale(8),
		weight = 600,
		extended = true,
...
function GM:DrawEntityInfo(entity, alpha, position)
	if (entity.IsPlayer(entity)) then
		...
		if (character) then
			local x, y = position.x, position.y - ScrH()*0.05
			local ty = 0
