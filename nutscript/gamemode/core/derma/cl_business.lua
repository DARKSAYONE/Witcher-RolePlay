...
function PANEL:setItem(itemTable)
	...
	self.price:SetFont("nutBusiness1")
	...
	self.name:SetFont("nutBusiness1")
...
function PANEL:Init()
	...
	self.itemList:SetSpaceX(3)
	self.itemList:SetSpaceY(5)
	self.itemList:SetMinimumSize(256, 800)
...
PANEL = {}
	function PANEL:Init()
		...
		self.current:SetFont("nutBusiness1")
		...
		self.total:SetFont("nutBusiness1")
		...
		line.Paint = function(this, w, h)
			...
			surface.DrawLine(0, 0, w + 128, 0)
		end

		self.final = self.data:Add("DLabel")
		self.final:SetFont("nutBusiness1")
		...
		self.finalGlow:SetFont("nutBusiness1")
	...
	function PANEL:onQuantityChanged()
		...
		self.current:SetText("Ваши деньги: "..nut.currency.get(money))
		self.total:SetText("- "..nut.currency.get(price))
		self.final:SetText("Остаток: "..nut.currency.get(money - price))
	...
	function PANEL:setCart(items)
		...
		for k, v in SortedPairs(items) do
			...
			if (itemTable) then
				...
				slot.name:SetSize(360, 32)
				slot.name:SetFont("nutBusiness1")
				...
				slot.quantity:SetFont("nutBusiness2")
...
hook.Add("CreateMenuButtons", "nutBusiness", function(tabs)
	if (LocalPlayer():IsAdmin() or LocalPlayer():getChar():hasFlags("b")) then
