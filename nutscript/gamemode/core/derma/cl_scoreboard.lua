local PANEL = {}
	...
	function PANEL:addPlayer(client, parent)
		...
		slot.model:SetModel(client:getChar():getModel(), client:GetSkin())
		...
		slot.desc:SetFont("nutGenericFont")
		...
		--	end
			...
			if (self.lastModel != model or self.lastSkin != skin) then
				...
				--if client:IsProne() then
				--	mdl = client:getChar():getModel()
				--end
				self.model:SetModel(mdl, client:GetSkin())
