local PANEL = {}
	function PANEL:Init()
		...
		self.model.copyLocalSequence = false
		...
		self.save = self.info:Add("DButton")
		self.save:Dock(TOP)
	...
	function PANEL:setup()
		...
		self.desc.OnChange = function()
			self.save:SetEnabled(true)
		end
		self.save.DoClick = function()
			nut.command.send("chardesc", self.desc:GetText())
			self.save:SetDisabled(true)
		end
		...
		--if LocalPlayer():IsProne() or !model then
		--	model = LocalPlayer():getChar():getModel()
		--end
		...
		for k, v in SortedPairsByMemberValue(nut.attribs.list, "name") do
			...
			bar.Think = function()
				bar.deltaValue = math.Approach(bar.deltaValue, bar.value, math.huge)
				bar:setText(L(v.name .. " = " .. math.Round(LocalPlayer():getChar():getAttrib(k, 0), 3)))
