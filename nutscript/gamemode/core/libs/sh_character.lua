...
do
	...
	nut.char.registerVar("model", {
		...
		onDisplay = function(panel, y)
			...
			scroll:SetSize(panel:GetWide(), 350)
			scroll:SetPos(25, y)
			...
			dpanel:SetPos(25, y)
			panel.payload.spawnplace = "Случайное"

			local DComboBox = dpanel:Add( "DComboBox" )
			...
			DComboBox.OnSelect = function( panel1, index, value )
				panel.payload.spawnplace = value
			end]]
	...
	--[[
	nut.char.registerVar("spawnplace", {
		field = "_spawnplace",
		index = 4,
		onDisplay = function(panel, y)
			panel.payload.spawnplace = "Случайное"
			local scroll = panel:Add("DPanel")
			scroll:SetSize(panel:GetWide() / 2, 20)
			scroll:SetPos(25, y)
			...
			local DComboBox = scroll:Add( "DComboBox" )
			DComboBox:Dock(TOP)
			DComboBox:SetSize( 100, 20 )
			DComboBox:AddChoice( "Майенна (Темерия)" )
			DComboBox:AddChoice( "Подворенга (Нильфгаард)" )
...
--]]
	...
	nut.char.registerVar("attribs", {
		...
		onDisplay = function(panel, y)
			...
			container:SetPos(25, y)
			container:SetWide(panel:GetWide() - 66)
...
if (CLIENT) then
	function nut.maxchars()
		local data = nut.localData and nut.localData["maxChars"]
...
do
	if (SERVER) then
		...
		netstream.Hook("charCreate", function(client, data)
			...
			nut.char.create(data, function(id, data)
				if (IsValid(client)) then
					...
					hook.Run("OnCharCreated", client, nut.char.loaded[id], data)
				end
			end)
