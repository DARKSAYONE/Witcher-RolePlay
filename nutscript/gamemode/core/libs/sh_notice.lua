if (SERVER) then
	...
	function nut.util.notify(message)
		...
		notice:SetPos(scrW - (notice:GetWide() + 4)-40, (i - 1) * (notice:GetTall() + 4) + 40)
		...
		local function OrganizeNotices()
			for k, v in ipairs(nut.notices) do
				v:MoveTo(scrW - (v:GetWide() + 4)-40, (k - 1) * (v:GetTall() + 4) + 40, 0.15, (k / #nut.notices) * 0.25, nil)
				v:AlphaTo(255, 0.5, 0)
			end
		end
			...
			if (IsValid(notice)) then
				...
				for k, v in ipairs(nut.notices) do
					if (v == notice) then
						...
						notice:AlphaTo(0, 0.15, 0.1, function()
							notice:Remove()
						end)
