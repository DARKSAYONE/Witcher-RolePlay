...
function nut.chat.register(chatType, data)
	...
	elseif (type(data.onCanHear) == "number") then
		...
		local range = data.onCanHear

		data.onCanHear = function(speaker, listener)
			-- Length2DSqr is faster than Length2D, so just check the squares.
			if data.canHearAim then
				local trace = listener:GetEyeTraceNoCursor()
				return (speaker:GetPos():Distance(listener:GetPos()) <= range) or (speaker:GetPos():Distance(trace.HitPos) <= range)
			else
...
if (SERVER) then
	...
	function nut.chat.send(speaker, chatType, text, anonymous, receivers)
		...
		if (class and class.onCanSay(speaker, text) != false) then
			...
			netstream.Start(receivers, "cMsg", speaker, chatType, hook.Run("PlayerMessageSend", speaker, chatType, text, anonymous, receivers) or text, anonymous, speaker:getChar():getAttrib(chatType))
		end
	end
else
	-- Call onChatAdd for the appropriate chatType.
	netstream.Hook("cMsg", function(client, chatType, text, anonymous, skill)
		if (IsValid(client)) then
			...
			if (class) then
					...
					CHAT_CLASS = class
						class.onChatAdd(client, text, anonymous, skill)
					CHAT_CLASS = nil
			end
		end
	end)
	...
	hook.Add("InitializedConfig", "nutChatTypes", function()
		...
		nut.chat.register("ooc", {
			onCanSay =  function(speaker, text)
				...
				if (delay > 0 and speaker.nutLastOOC and !speaker:IsAdmin()) then
			...
			onChatAdd = function(speaker, text)
				...
				chat.AddText(usergroups[speaker:GetUserGroup()], Color(255, 50, 50), " [OOC] ", speaker, color_white, ": "..text)
			end,
			prefix = {"//", "/ooc"},
			noSpaceAfter = true,
			filter = "ooc"
		...
		nut.chat.register("looc", {
			onCanSay =  function(speaker, text)
				...
				-- Only need to check the time if they have spoken in OOC chat before.
				if (delay > 0 and speaker.nutLastLOOC and !speaker:IsAdmin()) then
			...
			onChatAdd = function(speaker, text)
				...
				chat.AddText(usergroups[speaker:GetUserGroup()], Color(255, 50, 50), "[LOOC] ", nut.config.get("chatColor"), speaker:Name()..": "..text)
			end,
			onCanHear = nut.config.get("chatRange", 280),
			prefix = {".//", "[[", "/looc"},
			noSpaceAfter = true,
