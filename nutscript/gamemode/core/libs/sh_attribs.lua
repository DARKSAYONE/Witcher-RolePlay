...
function nut.attribs.loadFromDir(directory)
	for k, v in ipairs(file.Find(directory.."/*.lua", "LUA")) do
		...
		ATTRIBUTE = nut.attribs.list[niceName] or {}
			...
			ATTRIBUTE.name = ATTRIBUTE.name or "Неизвестно"
			ATTRIBUTE.desc = ATTRIBUTE.desc or "Описание недоступно."
	...
	if (SERVER) then
		function charMeta:updateAttrib(key, value)
			...
			if (attribute) then
				...
				attrib[key] = math.Clamp((attrib[key] or 0) + value, 0, 100)
		...
		function charMeta:addBoost(boostID, attribID, boostAmount, time)
			...
			timer.Simple(time, function()
				if IsValid(self:getPlayer()) then
					self:removeBoost(boostID, attribID)
				end
			end)
